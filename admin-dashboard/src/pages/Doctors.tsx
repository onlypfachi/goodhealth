...
import { Stethoscope, Plus, Search, Filter, Calendar, Clock, Save, Trash2, Eye, X, Building2, EyeOff } from "lucide-react";
...
const Doctors = () => {
  ...
  const fetchDepartments = async () => {
    try {
      const token = localStorage.getItem('staffToken');
  const response = await fetch(`${API_BASE_URL}/api/doctors/departments/list`, {
        headers: {
      ...
      if (data.success && data.data) {
        ...
        console.log('Departments set:', data.data.length);
      } else {
      ...
      toast({
        title: "Error",
        description: "Could not connect to server. Please ensure the backend is running.",
        variant: "destructive",
      });
    }
  };
  ...
  const filterDoctors = () => {
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter(d => {
        const isOnline = d.last_login && new Date(d.last_login) > new Date(Date.now() - 5 * 60 * 1000);
  ...
  const response = await fetch(`${API_BASE_URL}/api/doctors`, {
      ...
      if (data.success) {
        ...
        setNewDoctor({ fullName: "", email: "", password: "", phone: "", departmentIds: [] });
  ...
  const handleViewProfile = async (doctor: any) => {
    try {
      const token = localStorage.getItem('staffToken');
  const response = await fetch(`${API_BASE_URL}/api/doctors/${doctor.user_id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleAddShift = async () => {
    if (!selectedDoctor || !editingShift.dayOfWeek || !editingShift.startTime || !editingShift.endTime) {
      toast({
        title: "Error",
    ...
    try {
      const token = localStorage.getItem('staffToken');
  const response = await fetch(`${API_BASE_URL}/api/doctors/${selectedDoctor.user_id}/schedules`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleRemoveShift = async (scheduleId: number) => {
    try {
      const token = localStorage.getItem('staffToken');
  const response = await fetch(`${API_BASE_URL}/api/doctors/${selectedDoctor.user_id}/schedules/${scheduleId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleDeleteDoctor = async () => {
    ...
    try {
      const token = localStorage.getItem('staffToken');
  const response = await fetch(`${API_BASE_URL}/api/doctors/${doctorToDelete.user_id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const getDoctorStatus = (doctor: any) => {
    ...
    if (doctor.is_online === 1) {
      return { label: "Online", variant: "default" as const, color: "bg-accent-green" };
    }
    ...
    <div className="space-y-6">
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[600px] bg-background max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Add a new doctor to the system. Employee ID will be auto-generated.
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
                ...
                <div className="border rounded-lg p-3 space-y-2 max-h-[200px] overflow-y-auto bg-background">
                  {departments.length > 0 ? (
                    departments.map((dept) => (
                      <div key={dept.department_id} className="flex items-center space-x-2">
                        <Checkbox
                          ...
                          checked={newDoctor.departmentIds.includes(dept.department_id)}
                    ...
                    <p className="text-sm text-muted-foreground text-center py-4">
                      Loading departments... If this persists, please ensure departments are configured in the database.
                    </p>
                  )}
                </div>
              ...
              <div className="bg-accent-teal/10 border border-accent-teal/30 rounded-lg p-3">
                <p className="text-sm text-foreground">
                  ...
                  The doctor can use their email and password to login to the doctor dashboard.
                </p>
              </div>
            </div>
            <DialogFooter>
              ...
              <Button onClick={handleCreateDoctor} className="bg-accent-teal hover:bg-accent-teal/90">
                Create Account
              </Button>
            </DialogFooter>
          </DialogContent>
      ...
      <div className="space-y-4">
        {filteredDoctors.map((doctor) => {
          ...
          return (
            <Card key={doctor.user_id} className="transition-smooth hover:card-shadow-hover">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                  ...
                  <div className="flex items-center gap-4 min-w-[280px]">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{doctor.departments || 'No department'}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-6 flex-1">
                    <div>
                      ...
                      <p className="text-sm font-medium text-primary">{doctor.staff_id}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Today's Patients</p>
                      <p className="text-sm font-medium">{doctor.today_patients || 0}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Total Shifts</p>
                      <p className="text-sm font-medium">{doctor.total_shifts || 0}</p>
                    </div>
                  </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{doctorDetails.doctor.staff_id}</p>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Full Name</Label>
                    <p className="font-medium">{doctorDetails.doctor.full_name}</p>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Email</Label>
                    <p className="font-medium">{doctorDetails.doctor.email}</p>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Phone</Label>
                    <p className="font-medium">{doctorDetails.doctor.phone || 'N/A'}</p>
                  </div>
                  <div className="col-span-2">
                    <Label className="text-muted-foreground">Departments</Label>
                    <p className="font-medium">{doctorDetails.doctor.departments || 'No departments assigned'}</p>
                  </div>
                </div>
              </div>
                ...
                {doctorDetails.schedules.length > 0 ? (
                  <div className="space-y-2">
                    {doctorDetails.schedules.map((schedule: any) => (
                      <div key={schedule.schedule_id} className="p-3 border rounded-lg flex justify-between items-center">
                        <div className="flex items-center gap-3">
              ...
              <div className="space-y-3">
                ...
                {doctorDetails.appointments.length > 0 ? (
                  <div className="space-y-2 max-h-[200px] overflow-y-auto">
                    {doctorDetails.appointments.slice(0, 10).map((apt: any) => (
                      <div key={apt.appointment_id} className="p-3 border rounded-lg">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="font-medium">{apt.patient_name} ({apt.patient_id})</p>
                            <p className="text-sm text-muted-foreground">{apt.department_name}</p>
                          </div>
      ...
      <Dialog open={isShiftDialogOpen} onOpenChange={setIsShiftDialogOpen}>
        <DialogContent className="sm:max-w-[600px] bg-background">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedDoctor && `Managing shifts for ${selectedDoctor.full_name}`}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {/* Current Shifts */}
            <div>
              <h4 className="text-sm font-semibold mb-3 text-foreground">Current Shifts</h4>
              <div className="space-y-2 max-h-[200px] overflow-y-auto">
                {doctorDetails && doctorDetails.schedules.length > 0 ? (
                  doctorDetails.schedules.map((shift: any) => (
                    <div key={shift.schedule_id} className="flex items-center justify-between p-3 border border-border rounded-lg bg-secondary/30">
                      <div className="flex items-center gap-3">
                        <Calendar className="w-4 h-4 text-primary" />
                        <div>
                          <p className="font-medium text-sm text-foreground">{shift.day_of_week}</p>
                          <p className="text-xs text-muted-foreground">
            ...
            <div className="border-t border-border pt-4">
              <h4 className="text-sm font-semibold mb-3 text-foreground">Add New Shift</h4>
              <div className="grid gap-3">
                <div className="grid gap-2">
                  <Label>Day of Week</Label>
                  <Select value={editingShift.dayOfWeek} onValueChange={(value) => setEditingShift({ ...editingShift, dayOfWeek: value })}>
                ...
                <Button onClick={handleAddShift} className="gap-2 bg-primary hover:bg-primary-hover w-full">
              ...
              <div className="flex items-center gap-4 p-4 bg-destructive/10 rounded-lg border border-destructive/20">
                <Avatar className="h-12 w-12">
                  <AvatarFallback className="bg-destructive/20 text-destructive">
                    {doctorToDelete.full_name?.split(' ').map((n: string) => n[0]).join('').toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <p className="font-semibold">{doctorToDelete.full_name}</p>
                  <p className="text-sm text-muted-foreground">{doctorToDelete.email}</p>
                  <p className="text-sm text-muted-foreground">Staff ID: {doctorToDelete.staff_id || 'N/A'}</p>
                </div>
              </div>
              <div className="space-y-2 text-sm">
                <p className="font-semibold text-destructive">⚠️ Warning: This action cannot be undone!</p>
                <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                  <li>The doctor will be permanently removed from the database</li>
                  <li>They will no longer be able to access the system</li>
                  <li>All their schedules and department assignments will be deleted</li>
